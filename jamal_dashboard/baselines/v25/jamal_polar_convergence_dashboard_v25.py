...
def parse_infout(infout_path: Path) -> Dict:
    ...
    for line in text:
        ...
        if "XREF[m]:" in line:
            ...
            if len(nums) >= 3:
                meta["xref"] = float(nums[0])
                meta["yref"] = float(nums[1])
                meta["zref"] = float(nums[2])
...
def write_json(path, summaries, rows, history, adf_data, drag_rise_data, provenance, integrity_checks):
    payload = {
        "summaries": summaries,
        "results": rows,
        "history": history,
...
def make_html(summaries, conv_rows, history, adf_data, drag_rise_data, provenance, integrity_checks):
...
Plotly.newPlot = function(gd, data, layout, config) {{
  return originalPlotlyNewPlot(gd, data, mixedTypographyLayout(layout), config);
}};
...
def write_html(path, summaries, conv_rows, history, adf_data, drag_rise_data, provenance, integrity_checks):
    Path(path).write_text(make_html(summaries, conv_rows, history, adf_data, drag_rise_data, provenance, integrity_checks), encoding="utf-8")
...
def main():
    ...
    write_html(html_path, all_summaries, all_conv_rows, all_history, adf_data, drag_rise_data, provenance, integrity_checks)
    write_json(json_path, all_summaries, all_conv_rows, all_history, adf_data, drag_rise_data, provenance, integrity_checks)
